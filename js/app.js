...
let productRepository = (function () {
  ...
  function searchProductByPrice() {
    ...
    const filteredProductsByPrice = products.filter((product) => {
      if (
        minInputElement.valueAsNumber <= product.Preis &&
        product.Preis <= maxInputElement.valueAsNumber
      ) {
        return true;
      }
...
document
  .querySelectorAll(".min-input, .max-input, .min-range, .max-range")
  .forEach((priceSlider) => {
    priceSlider.addEventListener("change", () =>
      productRepository.searchProductByPrice(),
    );
  });
